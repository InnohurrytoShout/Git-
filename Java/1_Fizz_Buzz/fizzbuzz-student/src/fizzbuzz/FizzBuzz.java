...
public class FizzBuzz {
	...
	public String getValue(int n) {
		boolean Fizzboolean = false;
		boolean Buzzboolean = false;
		int Number404 = n;
		int result;
		int Value = 10;
		while(Number404 != 0){
			if(Number404 < 10) {
				...
				result = Number404 % Value;
			}
			
			if(result==fizzNumber)
			{
				Fizzboolean = true;
				
			}
			if(result==buzzNumber) 
			{
				Buzzboolean = true;
			}
			Number404 = Number404 / Value;
		};
		if(n % fizzNumber == 0) {
			Fizzboolean = true;
		}
		...
		else if(!Fizzboolean && Buzzboolean) 
		{
			return"buzz";
		}
		else if(Fizzboolean && Buzzboolean)
	...
	public String[] getValues(int start, int end) {
		int Number101 = start;
		String[] answerArray = new String [1 + end -start];
		for(int i = 0 ; i < answerArray.length ; i++) {
			answerArray[i] = getValue(Number101);
			Number101 = Number101 + 1;			
		}
		return answerArray;
	}
}